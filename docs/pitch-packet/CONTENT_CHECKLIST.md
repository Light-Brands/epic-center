<[Home](../../README.md) | [Pitch Packet](./README.md) | [Full Strategy](./DIGITAL_PITCH_PACKET.md) | [Quick Start](./QUICK_START.md) | [Financials](./FINANCIAL_MODEL_GUIDE.md)>

---

# Pitch Packet Content Checklist

> *"Bringing the lost travelers home to gardens of their hearts and souls."*

## Overview

This checklist tracks all content assets needed for the digital pitch packet. Use this to ensure nothing is missed before launch.

**Source Document**: [Executive Summary v2.0](../strategy/EXECUTIVE_SUMMARY.md) - All content should align with and reference this document.

---

## 1. Written Content

### Executive Materials

- [ ] **One-Pager** (1 page PDF)
  - Company name & tagline
  - Problem statement (2-3 sentences)
  - Solution overview (2-3 sentences)
  - Market size
  - Business model summary
  - Traction highlights
  - Team overview
  - The ask
  - Contact information

- [ ] **Executive Summary** (2-3 pages)
  - Expanded problem/solution
  - Market opportunity detail
  - Product description
  - Go-to-market strategy
  - Financial highlights
  - Team backgrounds
  - Investment terms

- [ ] **Investment Highlights** (1 page)
  - 5-7 bullet points of key investment thesis
  - Why now
  - Why us
  - Return potential

### Pitch Narrative Sections (11 Sections - Aligned with Executive Summary v2.0)

- [ ] **Section 1: The Hook (Problem)**
  - Crisis of disconnection narrative
  - The gap wealthy individuals face (medical vs. luxury vs. biohacking vs. plant medicine)
  - Why no one offers all four together
  - Human story/example

- [ ] **Section 2: The Solution**
  - Transformational Epicenter one-sentence pitch
  - 8 Foundational Pillars (detailed from Executive Summary)
  - Transformation architecture concept
  - "From/To" transformation outcomes table

- [ ] **Section 3: Why Now**
  - Regulatory timeline (2020-2026)
  - Structural opportunity explanation
  - Why competitors can't build this
  - First-mover advantage

- [ ] **Section 4: Market Opportunity**
  - $5.6T market across 4 segments (with specific figures)
  - Ideal guest profile (successful person who senses something missing)
  - Guest personas (executives, healthcare providers, artists, parents, seekers, survivors)
  - Common characteristics checklist

- [ ] **Section 5: Business Model**
  - $2,000/day base rate
  - Program pricing ($14K-$56K+)
  - Revenue model: 10 rooms × 70% occupancy = ~$5.1M
  - Additional revenue streams (bio-optimization, digital platform, ambassador network)

- [ ] **Section 6: Traction**
  - Documentation ecosystem (30+ documents)
  - Technical architecture designed
  - AI Superintelligent System architecture
  - Location research complete (Quintana Roo)
  - 30-location global expansion strategy

- [ ] **Section 7: Competition**
  - Why competitors can't copy (detailed by type)
  - 5 layers of competitive defense
  - Feature comparison matrix
  - Moat description

- [ ] **Section 8: Go-to-Market**
  - 4-Phase long-term vision (Years 1-10)
  - Guest journey arc (Awakening → Immersion → Integration → Embodiment)
  - Acquisition channels breakdown
  - 30+ locations target

- [ ] **Section 9: Financials**
  - Year 1: ~$3.6M (120+ guests, 50% occupancy)
  - Year 3: ~$5.1M (170+ guests, 70% occupancy)
  - Year 5: ~$11.7M (300+ guests, 2 locations)
  - Success metrics and targets

- [ ] **Section 10: Team**
  - Founder bios (detailed)
  - Co-founder bios
  - Key team bios
  - Advisor bios
  - Key hires planned (CEO, CMO, Medical Director)
  - Talent lock-up strategy

- [ ] **Section 11: The Ask**
  - $5M raise amount
  - Use of funds breakdown (40/30/15/10/5%)
  - Milestones enabled
  - Key risks and mitigations
  - Terms overview

### FAQ Content

- [ ] **Common Questions & Answers** (20-30 Q&As)
  - Market questions
  - Product questions
  - Financial questions
  - Team questions
  - Risk questions
  - Legal/regulatory questions
  - Technical questions
  - Operational questions

---

## 2. Financial Materials

### Core Financial Model

- [ ] **Full Financial Model** (Excel/Google Sheets)
  - Revenue projections (5 years)
  - Cost structure
  - P&L statements
  - Cash flow projections
  - Balance sheet
  - Key assumptions (clearly labeled)
  - Sensitivity analysis
  - Scenario modeling (bear/base/bull)

### Financial Summaries

- [ ] **Unit Economics One-Pager**
  - Average revenue per guest
  - Cost of delivery breakdown
  - Gross margin
  - Customer acquisition cost
  - Lifetime value
  - LTV:CAC ratio
  - Payback period

- [ ] **Use of Funds Breakdown**
  - Category allocations
  - Specific line items
  - Timeline for deployment
  - Milestone mapping

- [ ] **5-Year Projection Summary**
  - Revenue trajectory
  - EBITDA progression
  - Key milestones
  - Headcount plan

- [ ] **Cap Table**
  - Current ownership
  - Previous funding (if any)
  - Pro-forma post-raise
  - Option pool

### Interactive Model Data

- [ ] **Adjustable Assumptions**
  - Capacity parameters
  - Occupancy rates
  - Pricing tiers
  - Cost variables
  - Growth rates
  - Location expansion

---

## 3. Video Content

### Core Videos

- [ ] **Founder Story** (2-3 minutes)
  - Personal connection to mission
  - Why this problem matters
  - Vision for the future
  - Call to action
  - *Style: Authentic, personal, compelling*

- [ ] **Vision Sizzle Reel** (60-90 seconds)
  - Emotional opening
  - Problem visualization
  - Solution reveal
  - Future state imagery
  - Brand moment
  - *Style: Cinematic, inspiring, professional*

- [ ] **Platform/Product Demo** (3-5 minutes)
  - Guest app walkthrough
  - Admin dashboard preview
  - AI capabilities
  - Integration demonstration
  - *Style: Clean, professional, informative*

### Team Videos

- [ ] **Co-Founder Video** (60 seconds)
- [ ] **Medical Director Video** (60-90 seconds)
- [ ] **CTO/Technical Lead Video** (60 seconds)
- [ ] **[Other Key Team Member] Video** (60 seconds)

### Advisor/Social Proof Videos

- [ ] **Advisor Endorsement 1** (30-60 seconds)
- [ ] **Advisor Endorsement 2** (30-60 seconds)
- [ ] **Industry Expert Endorsement** (30-60 seconds)

### Optional/Stretch Videos

- [ ] **Market Opportunity Explainer** (2 minutes)
- [ ] **Day-in-the-Life (Guest Journey)** (3 minutes)
- [ ] **Facility Vision/Render Walkthrough** (2 minutes)

---

## 4. Visual Assets

### Brand Assets

- [ ] **Logo** (multiple formats)
  - Full color (light background)
  - Full color (dark background)
  - White version
  - Icon only
  - *Formats: SVG, PNG (multiple sizes)*

- [ ] **Brand Guidelines Reference**
  - Color palette (hex codes)
  - Typography specifications
  - Usage guidelines

### Pitch Graphics

- [ ] **8 Pillars Diagram**
  - Interactive/animated version
  - Static version for documents

- [ ] **Market Size Visualization**
  - TAM/SAM/SOM circles
  - Growth chart

- [ ] **Competitive Matrix**
  - Comparison chart graphic
  - Positioning map

- [ ] **Revenue Model Diagram**
  - Revenue streams visual
  - Unit economics breakdown

- [ ] **Timeline/Roadmap**
  - Company milestones
  - Product roadmap
  - Expansion plan

- [ ] **Team Org Chart**
  - Current structure
  - Planned structure

- [ ] **Guest Journey Map**
  - Pre-care → On-site → Post-care flow
  - Touchpoints visualization

### Photography

- [ ] **Founder Headshots** (professional)
- [ ] **Team Headshots** (professional)
- [ ] **Advisor Headshots** (from their profiles)
- [ ] **Facility Renders/Concepts** (if available)
- [ ] **Nature/Wellness Imagery** (for mood/atmosphere)

### Iconography

- [ ] **Custom icons for 8 pillars**
- [ ] **Feature icons** (for product sections)
- [ ] **Process icons** (for guest journey)

---

## 5. Data Room Documents

### Executive

- [ ] One-Pager.pdf
- [ ] Executive-Summary.pdf
- [ ] Investment-Highlights.pdf

### Financials

- [ ] Financial-Model.xlsx
- [ ] Unit-Economics.pdf
- [ ] 5-Year-Projections.pdf
- [ ] Use-of-Funds.pdf
- [ ] Cap-Table.pdf

### Market Research

- [ ] Market-Size-Analysis.pdf
- [ ] Competitive-Landscape.pdf
- [ ] Target-Customer-Research.pdf
- [ ] Industry-Report-1.pdf (third-party)
- [ ] Industry-Report-2.pdf (third-party)

### Product & Technology

- [ ] Technical-Architecture-Overview.pdf
- [ ] Platform-Roadmap.pdf
- [ ] HIPAA-Compliance-Framework.pdf
- [ ] Security-Overview.pdf
- [ ] Demo-Access-Credentials.pdf

### Operations

- [ ] Facility-Plans.pdf (if available)
- [ ] Operational-Model.pdf
- [ ] Regulatory-Strategy.pdf
- [ ] Risk-Assessment.pdf

### Team

- [ ] Detailed-Founder-Bios.pdf
- [ ] Team-Org-Chart.pdf
- [ ] Advisory-Board-Bios.pdf
- [ ] Key-Hire-Profiles.pdf
- [ ] Reference-Contacts.pdf

### Legal

- [ ] Corporate-Structure.pdf
- [ ] IP-Overview.pdf
- [ ] Term-Sheet-Draft.pdf (when ready)
- [ ] Material-Contracts-Summary.pdf

### Traction

- [ ] Letters-of-Intent.pdf (if available)
- [ ] Partnership-Summaries.pdf
- [ ] Waitlist-Metrics.pdf
- [ ] Press-Coverage.pdf (if available)

---

## 6. Technical Content

### For Technical Investors

- [ ] **Architecture Overview**
  - System diagrams
  - Tech stack justification
  - Scalability approach
  - Security framework

- [ ] **Development Status**
  - What's built
  - What's in progress
  - What's planned
  - Timeline

- [ ] **Demo Environment**
  - Access credentials
  - Feature highlights
  - Known limitations

---

## 7. Personalization Content

### Per-Investor Customization

For each target investor, prepare:

- [ ] **Custom greeting copy**
- [ ] **Relevant portfolio company references**
- [ ] **Thesis alignment points**
- [ ] **Personalized value proposition**

### Investor Categories

Prepare templates for:

- [ ] **Healthcare VCs**
- [ ] **Impact Investors**
- [ ] **Consumer/Hospitality VCs**
- [ ] **Angels**
- [ ] **Family Offices**
- [ ] **Strategic Investors**

---

## 8. Email Templates

### Outreach Sequence

- [ ] **Initial Outreach Email**
  - Cold intro template
  - Warm intro template
  - Re-engagement template

- [ ] **Follow-Up Emails**
  - After portal view (no data room)
  - After data room access
  - After meeting request
  - Post-meeting follow-up

- [ ] **Portal Access Email**
  - Welcome message
  - Navigation tips
  - Contact information

### Automated Emails

- [ ] **Magic Link Email**
- [ ] **Meeting Confirmation**
- [ ] **Document Access Notification**

---

## 9. Supporting Materials

### Reference & Research

- [ ] **Industry Reports** (licensed/purchased)
- [ ] **Market Research** (third-party validation)
- [ ] **Regulatory Guidance Documents**
- [ ] **Academic Studies** (relevant to approach)

### Social Proof

- [ ] **Advisor Commitment Letters**
- [ ] **Letters of Intent**
- [ ] **Partnership Agreements** (redacted if needed)
- [ ] **Press Mentions/Articles**
- [ ] **Testimonials** (if any beta/early users)

---

## Content Quality Standards

### Writing

- [ ] Professional, clear, jargon-free
- [ ] Consistent voice throughout
- [ ] Proofread by at least 2 people
- [ ] Legal review where appropriate
- [ ] Facts and figures verified

### Video

- [ ] Good lighting and audio
- [ ] Clean backgrounds
- [ ] Professional but authentic
- [ ] Captions available
- [ ] Optimized for web (compressed)

### Design

- [ ] On-brand colors and typography
- [ ] Consistent visual style
- [ ] Mobile-friendly formats
- [ ] High resolution (2x for retina)
- [ ] Accessible (color contrast, alt text)

### Documents

- [ ] Branded templates
- [ ] Consistent formatting
- [ ] PDF/A format for archival
- [ ] Reasonable file sizes
- [ ] Proper metadata

---

## Completion Tracking

| Category | Items | Complete | % |
|----------|-------|----------|---|
| Written Content | 16 | 0 | 0% |
| Financial Materials | 8 | 0 | 0% |
| Video Content | 12 | 0 | 0% |
| Visual Assets | 20 | 0 | 0% |
| Data Room Documents | 25 | 0 | 0% |
| Technical Content | 3 | 0 | 0% |
| Personalization | 8 | 0 | 0% |
| Email Templates | 8 | 0 | 0% |
| Supporting Materials | 8 | 0 | 0% |
| **Total** | **108** | **0** | **0%** |

**Note**: Executive Summary v2.0 is complete and serves as the source of truth for all pitch content.

---

## Priority Order

### Must-Have (Week 1-2)
1. One-Pager
2. Executive Summary
3. Financial Model
4. Founder Video
5. Core pitch narrative
6. Basic data room docs

### Should-Have (Week 3-4)
1. Full video suite
2. Interactive graphics
3. FAQ content
4. Personalization templates
5. Complete data room

### Nice-to-Have (Week 5+)
1. Sizzle reel
2. 3D renders
3. Industry reports
4. Extended video content

---

*Document Version: 2.0*
*Last Updated: January 2026*
*Aligned with: Executive Summary v2.0*

# Rancho Paraiso Oasis - Full Property Assessment
## Transformational Epicenter - Flagship Property Analysis

### Data Room Document 11 | Property Section

**TRANSFORMATIONAL EPICENTER**
**Luxury Medical Retreat | Plant Medicine & Bio-Optimization**

**Document Date:** January 2026
**Classification:** RESTRICTED - CONFIDENTIAL
**Prepared For:** Prospective Investors & Strategic Partners
**Property:** Rancho Paraiso Oasis (Kaax Cenote)
**Recommendation:** PROCEED

---

> **CONFIDENTIALITY NOTICE:** This document contains proprietary financial projections, acquisition terms, and strategic analysis prepared exclusively for authorized recipients participating in the Transformational Epicenter capital raise. Distribution, reproduction, or disclosure to unauthorized parties is strictly prohibited.

---

## TABLE OF CONTENTS

1. [Executive Summary](#1-executive-summary)
2. [Property Overview](#2-property-overview)
3. [Capacity Analysis](#3-capacity-analysis)
4. [Acquisition Cost Breakdown](#4-acquisition-cost-breakdown)
5. [Renovation Budget Breakdown](#5-renovation-budget-breakdown)
6. [Other Capital Costs](#6-other-capital-costs)
7. [Total Project Cost Summary](#7-total-project-cost-summary)
8. [Operating Cost Structure](#8-operating-cost-structure)
9. [Revenue Projections](#9-revenue-projections)
10. [Investment Returns](#10-investment-returns)
11. [Risk Factors](#11-risk-factors)
12. [Recommendation](#12-recommendation)

---

## 1. EXECUTIVE SUMMARY

The Rancho Paraiso Oasis (Kaax Cenote) is a 45,000 m2 jungle compound comprising 9 contiguous lots located in Tulum, Quintana Roo, Mexico. The property has been selected as the flagship site for Transformational Epicenter following a rigorous multi-property evaluation scoring **88/100** -- the highest of all candidates assessed.

### Key Metrics at a Glance

| Metric | Value |
|--------|-------|
| **Property Score** | 88/100 |
| **Acquisition Price** | $11,900,000 |
| **Total Renovation** | $1,000,000 |
| **Total Project Cost** | $16,823,500 |
| **Total Casitas (Full Build)** | 60 (phased 15→60 over 5 years) |
| **Average Daily Rate** | $2,000 |
| **Average Revenue Per Guest** | $26,600 |
| **Year 1 Revenue (60% Occupancy)** | $10,748,000 |
| **Year 1 Net Income** | $2,796,400 |
| **5-Year Total Revenue** | $125,880,000 |
| **Project IRR (Base)** | 78% |
| **Project IRR (Conservative)** | 66% |
| **Year 5 Enterprise Value (Base)** | ~$147,000,000 |
| **5-Year MOIC (Base)** | 7.5x |
| **5-Year MOIC (Conservative)** | 5.3x |

---

## 2. PROPERTY OVERVIEW

### 2.1 General Information

| Attribute | Detail |
|-----------|--------|
| **Property Name** | Rancho Paraiso Oasis (Kaax Cenote) |
| **Location** | Tulum, Quintana Roo, Mexico |
| **Property Type** | Jungle estate compound |
| **Configuration** | 9 contiguous lots |
| **Total Land Area** | 45,000 m2 (approx. 11.1 acres) |
| **Current Use** | Vacant / Development stage |
| **Zoning** | Rural / Low-density development |
| **Property Score** | 88/100 |
| **Natural Features** | Cenotes, mature jungle canopy, native flora |

### 2.2 Location Context

The property is situated in the Tulum corridor of the Riviera Maya, one of the fastest-growing luxury tourism destinations in the Western Hemisphere. Tulum has emerged as a global wellness destination, attracting HNW and UHNW travelers seeking authentic experiences in natural settings.

| Access Point | Distance | Travel Time |
|-------------|----------|-------------|
| Cancun International Airport (CUN) | ~130 km | 90 minutes |
| Tulum Town Center | ~15 km | 20 minutes |
| Tulum Beach Zone | ~18 km | 25 minutes |
| Hospital Galenia (Cancun) | ~130 km | 70-80 minutes |
| Playa del Carmen | ~65 km | 50 minutes |

### 2.3 Scoring Breakdown

| Category | Score | Max | Notes |
|----------|-------|-----|-------|
| Location & Setting | 14 | 15 | Pristine jungle, cenotes, complete immersion |
| Room Count & Capacity | 10 | 10 | 60 casitas (phased 15→60), maximum throughput |
| Medical Infrastructure Feasibility | 13 | 15 | Open layout enables full medical build-out |
| Construction & Renovation Scope | 8 | 10 | Significant but clean canvas approach |
| Land Size & Expansion Potential | 10 | 10 | 45,000 m2, 9 lots, unmatched expansion |
| Market Access & Transport | 6 | 8 | 90 min from CUN, jungle road |
| Regulatory Environment | 7 | 8 | Favorable rural zoning |
| Price & Value | 7 | 8 | Strong value per m2 |
| Revenue Potential | 9 | 10 | Highest IRR and MOIC of all candidates |
| Overall Investment Risk | 4 | 6 | Land backing, manageable risk profile |
| **TOTAL** | **88** | **100** | |

---

## 3. CAPACITY ANALYSIS

### 3.1 Room & Treatment Configuration

| Capacity Metric | Value |
|-----------------|-------|
| **Total Casitas (Full Build)** | 60 (phased 15→30→40→48→54→60) |
| **Year 1 Casitas** | 15→30 (23 effective) |
| **Average Program Duration** | 14 days (2 weeks) |
| **Annual Cycles (at 14-day avg)** | 26 cycles per year |
| **Max Guests Per Cycle (Full Build)** | 60 |
| **Theoretical Max Guests/Year (Full Build)** | 1,560 (60 x 26) |
| **Year 1 Target (60% Occupancy)** | 387 guests |
| **Total 5-Year Guests** | 4,537 |
| **Year 5 Revenue** | $35,461,000 |

### 3.2 Program Mix Assumptions

| Program | Duration | Mix % | Annual Guests (Yr 1) |
|---------|----------|-------|----------------------|
| 7-Day Reset | 7 days | 20% | 77 |
| 14-Day Interruption | 14 days | 40% | 155 |
| 21-Day Recalibration | 21 days | 25% | 97 |
| 28-Day Transformation | 28 days | 15% | 58 |
| **Total** | | **100%** | **387** |

### 3.3 Phased Capacity Ramp

| Year | Casitas (Start→End) | Effective Rooms | Est. Guests | Notes |
|------|---------------------|-----------------|-------------|-------|
| **Year 1** | 15→30 | 23 | 387 | Initial build-out, core medical + treatment |
| **Year 2** | 30→40 | 35 | 729 | Expansion phase, growing occupancy |
| **Year 3** | 40→48 | 44 | 974 | Continued expansion |
| **Year 4** | 48→54 | 51 | 1,169 | Approaching full scale |
| **Year 5** | 54→60 | 57 | 1,278 | Full build-out approaching 60 casitas |

---

## 4. ACQUISITION COST BREAKDOWN

### 4.1 Total Acquisition Budget

| Line Item | Amount | % of Acquisition |
|-----------|--------|------------------|
| Purchase Price | $11,900,000 | 100.0% |
| **TOTAL ACQUISITION** | **$11,900,000** | **100.0%** |

*Note: Closing costs, legal fees, due diligence, and fideicomiso setup are included in the $11.9M all-in acquisition price.*

### 4.2 Acquisition Notes

- **Fideicomiso:** Required trust structure for foreign ownership of Mexican real estate within the restricted zone (100 km from borders, 50 km from coastline). Annual maintenance approximately $2,000-3,000/year.
- **Currency Hedge:** Forward contract to lock USD/MXN exchange rate between letter of intent and closing to mitigate peso volatility.
- **Due Diligence:** Includes environmental assessment, structural survey, title search, zoning verification, and boundary survey across all 9 lots.
- **Title Insurance:** Protects against defects in title, liens, or encumbrances discovered post-closing.

---

## 5. RENOVATION BUDGET BREAKDOWN

### 5.1 Renovation Line Items

The $1,000,000 renovation budget covers essential modifications to prepare the existing compound for medical operations. The property's existing infrastructure reduces the scope of conversion work required compared to greenfield development.

| # | Category | Estimated Amount | Notes |
|---|----------|-----------------|-------|
| 1 | Medical infrastructure upgrades | $400,000 | ICU-grade electrical, plumbing, HVAC for treatment rooms |
| 2 | Treatment/ceremony room conversion | $250,000 | Dedicated ceremony spaces, soundproofing, climate control |
| 3 | Kitchen and hospitality upgrades | $150,000 | Medical diet capabilities, nutritional prep |
| 4 | Generator/backup power | $100,000 | Medical-grade power with automatic transfer switch |
| 5 | General facility improvements | $100,000 | Group spaces, outdoor treatment areas, landscaping |
| | **TOTAL RENOVATION** | **$1,000,000** | |

### 5.2 Renovation Strategy

The renovation converts an undeveloped jungle estate into a medical-grade wellness facility while preserving the natural environment that is central to the healing experience. Key design principles:

- **Medical-first infrastructure:** All systems designed to support ICU-grade operations
- **Nature integration:** Treatment spaces incorporate jungle views and natural ventilation
- **Phased execution:** 15 casitas in Year 1, scaling to 60 casitas by Year 5 (15→30→40→48→54→60)
- **Sustainable design:** Solar supplementation, rainwater harvesting, low-impact construction techniques

---

## 6. OTHER CAPITAL COSTS

### 6.1 Equipment & Pre-Opening

| Category | Amount | Description |
|----------|--------|-------------|
| Medical + Biohacking Equipment | $1,000,000 | ICU/monitoring, diagnostic, IV systems, biohacking equipment, pharmacy |
| Technology | $750,000 | Tech development |
| Pre-Opening Expenses | $200,000 | Staff recruitment, training, licensing, marketing launch, soft opening |
| Working Capital | $1,126,000 | Operating runway |
| **Subtotal Other Capital** | **$3,076,000** | |

### 6.2 Contingency

| Item | Calculation | Amount |
|------|-------------|--------|
| Contingency | Applied to renovation + equipment costs | $847,500 |

The 15% contingency provides a buffer against cost overruns, supply chain delays, currency fluctuations, and unforeseen construction challenges typical of jungle development projects in the Riviera Maya.

---

## 7. TOTAL PROJECT COST SUMMARY

### 7.1 Capital Stack

| Category | Amount | % of Total |
|----------|--------|------------|
| Acquisition | $11,900,000 | 70.7% |
| Renovation | $1,000,000 | 5.9% |
| Medical Equipment | $750,000 | 5.0% |
| FF&E | $600,000 | 4.0% |
| Technology | $750,000 | 4.5% |
| Pre-Opening | $200,000 | 1.3% |
| Working Capital | $1,126,000 | 7.5% |
| Contingency | $847,500 | 5.7% |
| **TOTAL PROJECT COST** | **$16,823,500** | **100.0%** |

### 7.2 Cost Efficiency Metrics

| Metric | Value |
|--------|-------|
| Cost per Casita (Full Build) | $249,225 |
| Cost per m2 of Land | $332 |
| Acquisition as % of Total | 41.1% |
| Hard Costs (Acquisition + Reno) as % | 68.2% |
| Soft Costs as % | 31.8% |

---

## 8. OPERATING COST STRUCTURE

### 8.1 Monthly Operating Budget

| Category | Monthly Cost | Annual Cost | % of Annual |
|----------|-------------|-------------|-------------|
| Staff Salaries & Benefits | $105,000 | $1,260,000 | 41.8% |
| Medical Staff & Supplies | $42,000 | $504,000 | 16.7% |
| Food & Beverage | $25,000 | $300,000 | 10.0% |
| Facility Maintenance | $18,000 | $216,000 | 7.2% |
| Utilities & Infrastructure | $12,000 | $144,000 | 4.8% |
| Insurance (Medical + Property) | $15,000 | $180,000 | 6.0% |
| Marketing & Guest Acquisition | $18,000 | $216,000 | 7.2% |
| Administrative & Legal | $8,250 | $99,000 | 3.3% |
| Technology & Systems | $4,000 | $48,000 | 1.6% |
| Contingency & Reserves | $4,000 | $48,000 | 1.6% |
| **TOTAL MONTHLY OPERATING** | **$251,250** | **$3,015,000** | **100.0%** |

### 8.2 Key Operating Metrics

| Metric | Value |
|--------|-------|
| Monthly Operating Cost | $251,250 |
| Annual Operating Cost | $3,015,000 |
| Direct Cost Per Guest | $4,629 |
| Fixed Cost Ratio | ~65% |
| Variable Cost Ratio | ~35% |
| Break-Even Guests/Year | ~25 (at $26,600 avg revenue) |
| Break-Even Occupancy | ~3% |

---

## 9. REVENUE PROJECTIONS

### 9.1 Five-Year Revenue Model

| Metric | Year 1 | Year 2 | Year 3 | Year 4 | Year 5 |
|--------|--------|--------|--------|--------|--------|
| **Casitas (Start→End)** | 15→30 | 30→40 | 40→48 | 48→54 | 54→60 |
| **Effective Rooms** | 23 | 35 | 44 | 51 | 57 |
| **Occupancy** | 60% | 75% | 80% | 85% | 86% |
| **Guests** | 387 | 729 | 974 | 1,169 | 1,278 |
| **Avg Revenue/Guest** | $26,600 | $26,600 | $26,600 | $26,600 | $26,600 |
| **Gross Revenue** | $10,748,000 | $20,440,000 | $27,426,000 | $31,805,000 | $35,461,000 |
| **EBITDA** | $4,672,000 | $11,599,089 | $16,372,641 | $19,188,249 | $21,512,217 |
| **Net Income** | $2,796,400 | $7,540,362 | $10,797,849 | $12,705,774 | $14,269,552 |
| **Cumulative Revenue** | $10,748,000 | $31,188,000 | $58,614,000 | $90,419,000 | **$125,880,000** |

### 9.2 Revenue Assumptions

| Assumption | Value | Basis |
|------------|-------|-------|
| Average Daily Rate | $2,000 | Comparable market analysis (see Document 13) |
| Average Program Duration | 13.3 days | Weighted by program mix |
| Average Revenue Per Guest | $26,600 | ADR x avg duration |
| Annual Rate Escalation | 0% (conservative) | Flat pricing assumed for base case |
| Operating Cost Escalation | 5%/year | Inflation and scaling adjustment |

---

## 10. INVESTMENT RETURNS

### 10.1 Return Metrics

| Metric | Base Case | Conservative Case | Aggressive Case |
|--------|-----------|-------------------|-----------------|
| **Project IRR** | **78%** | **66%** | **87%** |
| **Year 5 Enterprise Value** | **~$147,000,000** | -- | -- |
| **5-Year MOIC** | **7.5x** | **5.3x** | **9.6x** |
| Year 1 Cash-on-Cash Return | 37.4% | 31.2% | 43.6% |
| Payback Period | ~2.7 years | ~3.2 years | ~2.2 years |
| 5-Year Cumulative Net Income | $48,109,937 | $37,585,556 | $52,619,778 |

### 10.2 Net Present Value Analysis

| Discount Rate | NPV (Base Case) | NPV (Conservative) |
|---------------|------------------|---------------------|
| 10% | $22,854,000 | $18,678,000 |
| 12% | $20,612,000 | $16,743,000 |
| 15% | $17,689,000 | $14,218,000 |

### 10.3 Sensitivity Analysis

| Scenario | Occupancy Adjustment | IRR Impact | MOIC Impact |
|----------|---------------------|------------|-------------|
| Aggressive Case | Higher occupancy ramp | 87% | 9.6x |
| Base Case | As modeled | 78% | 7.5x |
| Conservative Case | Lower occupancy ramp | 66% | 5.3x |
| Bear Case (-20% occupancy) | -20% across all years | 52% | 3.5x |
| ADR Sensitivity (+$500/day) | Flat occupancy, +$500 ADR | +15-20% | +1.5x |
| ADR Sensitivity (-$500/day) | Flat occupancy, -$500 ADR | -18-22% | -1.3x |

---

## 11. RISK FACTORS

### 11.1 Property-Specific Risks

| Risk | Probability | Impact | Mitigation |
|------|-------------|--------|------------|
| **Renovation cost overrun** | Moderate | Medium | 15% contingency ($847,500), phased approach reduces exposure |
| **Construction timeline delay** | Moderate | Medium | Phase 1 opens at 15 casitas; expansion to 60 casitas phased over 5 years while revenue generates |
| **Jungle infrastructure challenges** | Moderate | Low-Medium | Generator power, water treatment, experienced local contractors |
| **Hurricane/weather damage** | Low | High | Comprehensive insurance, jungle canopy provides wind break, reinforced construction |
| **Title/ownership complications** | Low | High | Title insurance, fideicomiso structure, comprehensive due diligence |
| **Road access limitations** | Low | Medium | Multiple access routes, maintained jungle roads, helicopter as backup |
| **Distance from hospital** | Ongoing | Critical | On-site ICU, helicopter evacuation, 24/7 medical staff |
| **Regulatory changes** | Low | Medium | Legal counsel, favorable current framework, community relations |
| **Currency fluctuation (MXN/USD)** | Moderate | Low-Medium | Revenue denominated in USD, hedge on acquisition |

### 11.2 Market & Operational Risks

| Risk | Probability | Impact | Mitigation |
|------|-------------|--------|------------|
| **Occupancy below projections** | Low-Moderate | Medium | Break-even at ~3% (~25 guests/year), substantial margin of safety |
| **Competitive entry** | Moderate | Low-Medium | Medical-grade + plant medicine moat, first-mover advantage |
| **Reputational risk (adverse event)** | Low | Critical | ICU capability, screening protocols, insurance, crisis management plan |
| **Staff recruitment in Tulum** | Moderate | Medium | Competitive compensation, staff housing on-site, Mexico City recruitment |
| **Supply chain for medical equipment** | Low-Moderate | Low | Cancun logistics hub, pre-purchase critical items, backup suppliers |

---

## 12. RECOMMENDATION

### PROCEED WITH ACQUISITION

Based on the comprehensive assessment presented in this document, the recommendation is to **proceed with acquisition and development** of the Rancho Paraiso Oasis (Kaax Cenote) as the Transformational Epicenter flagship property.

**Supporting rationale:**

1. **Highest score** of all evaluated properties at 88/100
2. **Strongest financial returns** with 78% IRR and 7.5x MOIC (base case), producing ~$147M enterprise value at Year 5
3. **Exceptional capacity** of 60 casitas (phased 15→60) supporting $10.7M Year 1 revenue
4. **Best cost efficiency** at $249,225 per casita at full build
5. **Unmatched expansion potential** on 45,000 m2 across 9 lots
6. **Perfect brand alignment** with jungle/cenote setting for healing environment
7. **Phased de-risk strategy** with casita build-out scaling from 15 to 60 over 5 years
8. **Low break-even** of ~3% occupancy (~25 guests/year) provides substantial safety margin

### Immediate Next Steps

| Step | Timeline | Responsibility |
|------|----------|----------------|
| Execute Letter of Intent | Week 1-2 | Legal counsel |
| Begin comprehensive due diligence | Week 2-4 | Legal + technical team |
| Initiate fideicomiso process | Week 3-6 | Mexican legal counsel |
| Finalize renovation design | Week 4-8 | Architecture + medical design team |
| Close acquisition | Week 8-12 | All parties |
| Begin Phase 1 renovation | Post-closing | General contractor |

---

*Document 11 of Data Room Property Section*
*Transformational Epicenter -- January 2026*
*RESTRICTED - CONFIDENTIAL*
*This document contains proprietary information intended solely for authorized investors and strategic partners.*
